...
(0, vitest_1.describe)('<Puff />', () => {
    (0, vitest_1.afterEach)(solid_testing_library_1.cleanup);
    (0, vitest_1.test)('default params', async () => {
        // arrange
        (0, solid_testing_library_1.render)(() => <Spinner_1.Spinner type={types_1.SpinnerType.puff}/>);
        // act
        ...
        // assert
        (0, vitest_1.expect)(width).toBe('44');
        (0, vitest_1.expect)(height).toBe('44');
        (0, vitest_1.expect)(class_).toBe('');
        (0, vitest_1.expect)(viewBox).toBe('0 0 44 44');
        (0, vitest_1.expect)(testName).toBe('puff');
    });
    (0, vitest_1.test)('change params', async () => {
        // arrange
        (0, solid_testing_library_1.render)(() => (<Spinner_1.Spinner width={50} height="60" class="spinner" style={{ margin: '10px' }} type={types_1.SpinnerType.puff} color="red"/>));
        // act
        const svg = await solid_testing_library_1.screen.findByRole('img');
        const width = svg.getAttribute('width');
        const height = svg.getAttribute('height');

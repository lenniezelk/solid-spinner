...
(0, vitest_1.describe)('<Rings />', () => {
    (0, vitest_1.afterEach)(solid_testing_library_1.cleanup);
    (0, vitest_1.test)('default params', async () => {
        // arrange
        (0, solid_testing_library_1.render)(() => <Spinner_1.default type={types_1.SpinnerType.rings}/>);
        // act
        ...
        // assert
        (0, vitest_1.expect)(width).toBe('45');
        (0, vitest_1.expect)(height).toBe('45');
        (0, vitest_1.expect)(class_).toBe('');
        (0, vitest_1.expect)(viewBox).toBe('0 0 45 45');
        (0, vitest_1.expect)(testName).toBe('rings');
    });
    (0, vitest_1.test)('change params', async () => {
        // arrange
        (0, solid_testing_library_1.render)(() => (<Spinner_1.default width={50} height="60" class="spinner" style={{ margin: '10px' }} type={types_1.SpinnerType.rings} color="red"/>));
        // act
        const svg = await solid_testing_library_1.screen.findByRole('img');
        const width = svg.getAttribute('width');
        const height = svg.getAttribute('height');
